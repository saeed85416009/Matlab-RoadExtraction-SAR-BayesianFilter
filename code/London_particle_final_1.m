...
a=imread ( 'E:\mat\image\London.jpg');    %%---**---%%
b=rgb2gray(a);
x=double(b);
...
dtt=dt;
 ...
 r_optimum = 311 ;                       %% initialize row for priori estimate   (pixel)                    %%---**---%%
 c_optimum = 324;                         %% initialize column for  priori estimate (pixel)                %%---**---%%
...
particle_new_column = c_optimum ; 
 ...
 PROILE_SOURCE =[ 93 90 89 102 100  ];        %% initialize reference profile                  %%---**---%%
 ...
 th_gray_eliminate = 105;  %% eliminate point that put over of road
 %%% for add
