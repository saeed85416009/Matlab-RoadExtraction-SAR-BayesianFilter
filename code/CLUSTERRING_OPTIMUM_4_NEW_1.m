function [ CLUSTER , SAMPLE_FOR_CLUSTER , SINGLE_POINT , FLAG_ANY_FIND_CLUSTERRING ,  test] =  CLUSTERRING_OPTIMUM_4_NEW_1 (SAMPLE , CLUSTER , SAMPLE_FOR_CLUSTER , SINGLE_POINT , SAMPLE_TOTAL , X_BIG , ANGLE ,  TH_ANGLE , TH_DIS , CLASS_NUMBER , CLASS , TH_ANGLE_FIRST , TH_DIS_FIRST )
...
for fi = ANGLE - TH_ANGLE_FIRST : ANGLE + TH_ANGLE_FIRST
    ...
    c_4 =  X_BIG(2) + dis * cosd ( fi );
       ...
       for s=1:size_sample(1)  %% search between sampls
                    ...
                    if (( round(SAMPLE(s,7)) == round(r_4) )  && ( round(SAMPLE(s,8)) == round(c_4) )) || (( round(SAMPLE(s,7)) == ceil(r_4) )  && ( round(SAMPLE(s,8)) == ceil(c_4) )) || (( round(SAMPLE(s,7)) == floor(r_4) )  && ( round(SAMPLE(s,8)) == floor(c_4) ))  %% if sightly point exist on sample 
