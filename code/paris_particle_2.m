 ...
 for i=1:k %% generate sample
  ...
  samp(i,2) = f_k_priori(i,2) +  s_new(i,2);
    ...
    if( x(round(samp(i,1)),round(samp(i,2))) <=  m )  %% eliminate rand point of out road 
          ...
          sample(sample_number,1) = f_k_priori(i,1) +  s_new(i,1);
          sample(sample_number,2) = f_k_priori(i,2) +  s_new(i,2);
          d=(sample(sample_number,2) - particle_new_column ) / sqrt(( (sample(sample_number,1) - particle_new_row  )^2 ) + ( (sample(sample_number,2) - particle_new_column ) ^2 ) );
          angle_sample_new = asind(d);
          sample(sample_number,3) =  ( angle_sample_new * pi )/180 ;  
          sample(sample_number,4) = 0;
          angle_sample(sample_number,3) = ( sample(sample_number,3) * 180 )/ pi  ;
          fo_sample(sample_number,4) = ( sample(sample_number,4) * 180) / pi  ;
          
          
          tiz(round(sample(sample_number,1)),round(sample(sample_number,2))) = 255;
    else
        x(round(samp(i,1)),round(samp(i,2)))
    end

 end%%end generate sample
 
 sample(sample_number+1 : end , : ) = 0;
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% assign weight to particle 
 for j=1:sample_number %% assign weight to sample
        
          weight_particle(j) = 1/sample_number;      %% thats true because my sample now have sample_number ( not N or k )
 end %% end assign weight to sample
 
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 %%     create final x from sample and its weight
     particle_new_row = 0;
     particle_new_column = 0;
     for f=1:sample_number  %% create final x from sample and its weight
