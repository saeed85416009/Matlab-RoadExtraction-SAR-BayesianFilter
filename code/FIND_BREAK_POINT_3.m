function [ FLAG_EXCHANGE_PARTICLE , angle_break ] = FIND_BREAK_POINT_3 ( ROW_PARTICLE , COLOUMN_PARTICLE , ANGLE_OLD , GRAY_ELIMINATE_PARTICLE , WIDe , Error_Kalman , Iteration_intersection_particle , iteration_detour_particle)
...
%         ANGLE
        ...
        add = 1;
        angle_find(angle_find_branch,1) = min(ANGLE);
        angle_find(angle_find_branch,2) = max(ANGLE);
        ANGLE = [];
%         pause
...
if (branch >= min_number_of_branch_in_intersection) || ( flag_intersect == 0 )
    ...
    FUNCTION_PARTICLE_5( ROW_PARTICLE , COLOUMN_PARTICLE , ANGLE_OLD , GRAY_ELIMINATE_PARTICLE , WIDe , detour , 4 , Error_Kalman , Iteration_intersection_particle )
                                if flag_repetitive_place == 0
                                    FUNCTION_PARTICLE_5( ROW_PARTICLE , COLOUMN_PARTICLE , ANGLE_OLD , GRAY_ELIMINATE_PARTICLE , WIDe , detour , 2 , Error_Kalman ,  iteration_detour_particle )
                                end
