...
function [ CLUSTER , SAMPLE_FOR_CLUSTER , FLAG_ANY_FIND_CLUSTERRING ,  test] =  CLUSTERRING_OPTIMUM_3_2 (SAMPLE , SAMPLE_EXT , CLUSTER , SAMPLE_FOR_CLUSTER , SAMPLE_TOTAL , X_BIG , ANGLE ,  TH_ANGLE , TH_DIS , CLASS_NUMBER , TH_ANGLE_FIRST )
...
source_cluster(clus,1)=CLASS_NUMBER;                          %% class number
source_cluster(clus,7)=X_BIG(6);                   %% conditiont of sample point in class
source_cluster(clus,8)=X_BIG(7);         %% conditiont of sample point in class
source_cluster(clus,9)=X_BIG(8);      %% conditiont of sample point in class
...
for i=1:size_total_sample(1)
    if (round(SAMPLE_TOTAL(i,7)) == round(source_cluster(clus,7)) ) &&  (round(SAMPLE_TOTAL(i,8)) == round(source_cluster(clus,8) ))
            ...
            break;
                            ...
                            source_cluster(clus,2) = X_BIG(1);
                             source_cluster(clus,3) = X_BIG(2);
