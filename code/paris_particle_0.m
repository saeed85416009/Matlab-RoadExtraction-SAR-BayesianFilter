...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  for i=1:N+1       %%  N+1=size sample
      ...
      if( x(round(samp(i,1)),round(samp(j,2))) <=  m )  %% eliminate rand point of out road 
          ...
          angle_sample = asind(d);
          sample(k,3) =  ( angle_sample * pi )/180 ;  
          sample(k,4) = 0;
...
%  for i=1:k
%      sample(:,:,i) = mvnpdf(x_0,f_k_priori(:,i),sigma)
%  end
%  
 
% P_0_new=50*Q_0;
%  sample=mvnpdf(f_k_priori,transpose(x_0),P_0_new)
...
%%% x_k(i) = f_k(x_k(i)) + N ( x_k ; f_k(x_k(i)) , p_k )
 for i=1:k
  ...
  samp_new(i,2) = f_k_priori(i,2) +  s_new(i,2);
          ...
          angle_sample = asind(d);
          sample(i,3) =  ( angle_sample * pi )/180 ;  
          sample(i,4) = 0;
     ...
     for i=1:k
         particle_new_row=particle_new_row + (  weight_particle(i) * sample(i,1));
         particle_new_column=particle_new_column + (  weight_particle(i) * sample(i,2));
