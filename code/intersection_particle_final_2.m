...
a=imread ( 'E:\internet\uni\proje\matlab\2.png');    %%---**---%%
b=rgb2gray(a);
x=double(b);
...
dtt=dt;
 ...
 r_optimum = 265 ;                       %% initialize row for priori estimate   (pixel)                    %%---**---%%
 c_optimum = 144;                         %% initialize column for  priori estimate (pixel)                %%---**---%%
 particle_new_row = r_optimum ;
particle_new_column = c_optimum ; 

 angle_first_point =45 ;                                                                                                %%---**---%%
 ...
 PROILE_SOURCE =[ 25 20 18 16 19  ];        %% initialize reference profile                  %%---**---%%
 ...
 th_gray_eliminate = 40;  %% eliminate point that put over of road
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 class_number =1;
 angle = ( x_0(3) * 180)/pi ;
 [clusterring , sample_for_cluster , flag_find_clusterring ] = CLUSTERRING_OPTIMUM_2_4 (sample_for_cluster  , clusterring , sample_for_cluster , x_0 , angle , th_angle_particle_2, th_distance_clusterring , class_number , th_distance_clusterring );
  note='check'
  pause
...
%      pause
 ...
 [clusterring , sample_for_cluster , single_point , flag_find_clusterring ] = CLUSTERRING_OPTIMUM_2_NEW (remain_clusterring  , clusterring , sample_for_cluster , single_point , sample_for_cluster,  x_new  , th_angle_particle_2, th_distance_clusterring , class_number , th_angle_particle_first , th_angle_true_from_center);
...
%% section 5
 ...
 REGULARIZED_SAMPLE()
sample_for_cluster=[];
sample_for_cluster  = CREATE_SAMPLE_FOR_CLUSTERRING_2 ();
...
IMSHOW_IMAGE_2(remain , 2 , 3 , 1 )
     ...
     angle = ( x_total(class , 3) * 180)/pi 
      ...
      pause
      if time ==1
     [clusterring , sample_f_c , flag_find_clusterring ] = CLUSTERRING_OPTIMUM_3_1_1 (remain  , remain_ext , clusterring , sample_f_c , sample_for_cluster , x_total( class , : ) , angle , th_angle_particle_1, th_distance_clusterring , class , th_angle_particle_first_time , th_dis_particle_first_time  );
      else
    [clusterring , sample_f_c , flag_find_clusterring ] = CLUSTERRING_OPTIMUM_3_2 (remain  , remain_ext , clusterring , sample_f_c , sample_for_cluster , x_total( class , : ) , angle , th_angle_particle_1, th_distance_clusterring , class , th_angle_particle_first_time );
      end
     clusterring
     IMSHOW_IMAGE_2(clusterring , 7 , 8 , 2 )
...
clusterring = MEDIAN_ANGLE (clusterring , 9 , 12) ;                        

IMSHOW_IMAGE( clusterring , 7 , 8 )
...
%                  pause
                ...
                if flag_find_new_point == 0
                           [clusterring , sample_f_c , single_point , flag_find_clusterring ] = CLUSTERRING_OPTIMUM_4_NEW_1 (remain_clusterring  , clusterring , sample_f_c , single_point , sample_for_cluster ,x_new , angle , th_angle_particle_1, th_distance_clusterring , class_number , class , th_angle_particle_first_time , th_dis_particle_first_time);
                end
...
%                                                IMSHOW_IMAGE( yyyy , 2 , 3 )
                                            ...
                                            %     x_new
                                            %     t1=x_new(3)*180/pi
                                                [ x_new(3) , t3 ] = ANGLE_FIND_CLUSTERRING_TIME_1( x_new , remain_clusterring , 7 , 8  );
                                            %     t3
                                            %      t2=x_new(3)*180/pi
                                            %      pause
                                                end
                                                      [clusterring , sample_f_c , single_point , flag_find_clusterring ] = CLUSTERRING_OPTIMUM_4_NEW (remain_clusterring  , clusterring , sample_f_c , single_point , sample_for_cluster ,x_new , angle , th_angle_particle_1, th_distance_clusterring , class_number , class , th_angle_particle_first_time);
...
note='end'
 ...
 class_number = max(clusterring(:,1));
 clusterring = MEDIAN_ANGLE (clusterring , 9 , 16);
